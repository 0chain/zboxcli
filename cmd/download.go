...
func init() {
	...
	downloadCmd.Flags().BoolP("thumbnail (boolean)", "t", false, "pass this option to download only the thumbnail")
	...
	downloadCmd.Flags().BoolP("verifydownload (boolean)", "v", false, "pass this option to verify downloaded blocks")

	downloadCmd.Flags().Bool("live (boolean)", false, "start m3u8 downloader,and automatically generate media playlist(m3u8) on --localpath")
